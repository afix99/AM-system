...
export function PerformanceClient({ stores }: { stores: { id: string; name: string }[] }) {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div className="rounded-xl border border-white/[0.10] p-3 text-xs" style={{ background: "#0C1228" }}>
        <p className="font-semibold text-slate-200 mb-1">{payload[0]?.payload?.fullName || label}</p>
        {payload.map((p: any) => (
  ...
  return (
    <div className="p-4 md:p-6 space-y-5 max-w-4xl mx-auto w-full">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <p className="text-xs font-medium text-indigo-400/80 uppercase tracking-widest mb-0.5">Analytics</p>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="p-1.5 rounded-lg hover:bg-white/[0.06] text-slate-500 hover:text-slate-300 transition-colors border border-white/[0.07]"
        >
          <ChevronLeft size={16} />
        </button>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center text-slate-600 text-sm">No performance data for this month.</div>
      ) : (
        <>
          {/* Rankings table */}
          <Card>
            <CardHeader>
              <CardTitle>Store Rankings — {MONTHS[month - 1]} {year}</CardTitle>
            </CardHeader>
                ...
                <tbody className="divide-y divide-white/[0.04]">
                  {data.map((d) => (
                    <tr
                      ...
                      className={`cursor-pointer transition-colors hover:bg-white/[0.03] ${selectedStore === d.store.id ? "bg-indigo-950/30" : ""}`}
                    >
                      <td className="px-4 py-3">
                        <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                          d.rank === 1 ? "bg-yellow-950/60 text-yellow-400 border border-yellow-500/30" :
                          d.rank === 2 ? "bg-white/[0.06] text-slate-300 border border-white/[0.10]" :
                      ...
                      <td className="px-4 py-3 font-medium text-slate-200">{d.store.name}</td>
                      <td className="px-4 py-3 text-slate-400">{formatCurrency(d.totalSales)}</td>
                      <td className="px-4 py-3 text-slate-600">{formatCurrency(d.targetSales)}</td>
                      <td className="px-4 py-3">
...
function StaffRatings({ storeId, month, year }: { storeId: string; month: number; year: number }) {
  ...
  if (staffPerf.length === 0) return <p className="text-sm text-slate-600">No ratings recorded for this month.</p>;
  return (
    <div className="space-y-3">
      {staffPerf.map((sp) => (
        <div key={sp.id} className="flex items-center justify-between py-2 border-b border-white/[0.04] last:border-0">
          <div>
            <p className="text-sm font-medium text-slate-200">{sp.staff.name}</p>
            <p className="text-xs text-slate-600">{sp.staff.role}</p>
          </div>
          <div className="flex gap-0.5">
            {[1, 2, 3, 4, 5].map((star) => (
              <span key={star} className={`text-lg leading-none ${sp.rating >= star ? "text-yellow-400" : "text-white/[0.07]"}`}>★</span>
            ))}
          </div>
        </div>
      ))}
