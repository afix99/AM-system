...
export function MobileNav() {
  ...
  return (
    ...
    >
      {/* Floating sparkle marker that hovers over the active tab */}
      {activeIndex >= 0 && (
        <div
          ...
          {links.map(({ href, label, icon: Icon }, i) => {
            ...
            return (
              <Link
                ...
                className="flex-1 relative flex flex-col items-center justify-center py-2.5 gap-1 min-h-[60px] active:scale-90 transition-transform duration-150"
              >
                <Icon
                  size={active ? 22 : 19}
                  strokeWidth={active ? 2.6 : 1.7}
                  className={`transition-all duration-300 ${active ? "text-[#FFB088]" : "text-stone-500"}`}
